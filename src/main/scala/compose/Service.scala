...
case class Service(name: String, projectName: String, gitURL: String, gitBranch: String,
  ...
  private def sbtPackage(projectPath: Path) = {
    val sbtOpts = List("compile", "package", "publishLocal")
    val buildResult = %.`sbt`(sbtOpts)(projectPath)
    if (buildResult != 0) System.exit(buildResult)
  }
  ...
  def isNeedBuildLocally(realImage: String): Boolean = {
  ...
  def buildDependsProjects(context: Context, mvnProfile: String): Unit = {
  ...
  def getRealImage(gids: Map[String, String]): String = {
